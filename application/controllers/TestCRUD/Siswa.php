...
class Siswa extends CI_Controller {
  ...
  public function index(){
    ...
    $this->load->view('TestCRUD/index', $data);
  }
  ...
  public function ubah($nim){
    if($this->input->post('submit')){ // Jika user mengklik tombol submit yang ada di form
      if($this->SiswaModel->validation("update")){ // Jika validasi sukses atau hasil validasi adalah TRUE
        $this->SiswaModel->edit($nim); // Panggil fungsi edit() yang ada di SiswaModel.php
        redirect('siswa');
      }
    }
  ...
  public function hapus($nim){
    $this->SiswaModel->delete($nim); // Panggil fungsi delete() yang ada di SiswaModel.php
    redirect('siswa');
  }
}
